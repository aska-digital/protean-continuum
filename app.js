...
const API = './snapshot'
const STATIC_PANES = {"e1353130-c1a7-594a-95bc-23a7654713ed":{"synthetic-project-2/20260201_000001_bbb901":"./snapshot.e1353130-c1a7-594a-95bc-23a7654713ed__pane-1.json"}}
const POLL_MS = 15000
...
// synthetic-design's exact copy (UX-13/UX-14). `quiet` is a group, not a band value on a card.
const BAND_LABELS = {
...
// Mission-control copy (synthetic-design UX-01..UX-26) used verbatim. No score, no countdown, no guilt.
const MC_COPY = {
...
// synthetic-design UX-06: the three attention groups, in server order.
...
// The dense grid columns, in synthetic-design's locked order (UX-09).
...
// ── synthetic-coord action log (AL architecture, §5/§6) ─────────────────────────────────────────────
...
// synthetic-coord directive 2026-09-17: the action-log DEFAULT view is the external filter; the toggle
...
// belongs to synthetic-design (OPEN-1 / TH-L12).
...
function parseUrlState(search) {
  ...
  if (state.view === 'action_log') {
    ...
    // synthetic-coord 2026-09-17: absent or invalid scope on the URL means the default (external).
...
// synthetic-design UX-13/UX-14: the server's band key maps to one fixed label. An unknown key is shown
...
// synthetic-design UX-10: tier and confidence are separate, and neither is ever colour-only.
...
// MC-L7 / synthetic-design UX-09: declared owner, else the verified primary profile, else the explicit
...
// client-side relative rewrite (synthetic-design UX-25).
...
// MC-L6 / synthetic-design UX-11: a derived value is a real control that opens its claim source. A label
...
function renderCard(card, col, data) {
  ...
  if (!STATIC_MODE) rows.push(moveRow)
  ...
  return h('div', {
    ...
    draggable: STATIC_MODE ? 'false' : 'true',
      ...
      return h('div', {
        ...
        lane.length === 0
          ? h('div', { class: 'c-lane-empty' }, [
              ...
              // synthetic-design UX-19: an empty lane names the next recoverable action.
...
// ── evidence drawer (MC-L6 / synthetic-design UX-11/UX-12) ────────────────────────
...
function closeEvidence() {
  ...
  render()
  // Focus returns to the control that opened the drawer (synthetic-design UX-11/UX-25).
  requestAnimationFrame(() => {
...
// ── dense grid (MC-S3 / synthetic-design UX-08/UX-09) ─────────────────────────────
// The grid renders the SAME `/projects` items the lanes do — one request, one envelope, and no
// second project list. Column order is synthetic-design's locked order.
...
// ── staleness bands (MC-S4 / synthetic-design UX-13/UX-14) ────────────────────────
...
// ── Recovery Inbox (MC-S5 / synthetic-design UX-15..UX-18) ────────────────────────
...
// synthetic-design UX-22: active filters are shown from the SERVER's echoed values, never re-derived.
...
function renderEmpty(data) {
  // synthetic-design UX-20: an empty RESULT and an empty CORPUS are different states with different actions.
...
// ── session pane (synthetic-agent-6 D-SP-1..13) ───────────────────────────────────
...
function renderToolbar() {
  ...
  // synthetic-design UX-08: both views show the same filtered projects.
...
// Skin column captions (synthetic-design §4.10/§9): AL_COLUMNS[0..5] repeat the header captions
...
function renderActionLogFilters() {
  ...
  return h('div', { class: 'c-al-filters' }, [
    // synthetic-coord 2026-09-17: ONE control switching External ⇄ Full log. It patches only `scope`
...
function renderActionLog() {
  ...
  const nodes = [
    ...
    h('div', { class: 'c-al-bar' }, [
      ...
      // synthetic-coord 2026-09-17: the scope split is a SERVER field (additive counts keys) — rendered
...
function boot() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && state.evidence) {
      // synthetic-design UX-11/UX-25: Escape closes the drawer and returns focus to the opening control.
      e.preventDefault()
      closeEvidence()
